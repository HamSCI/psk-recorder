"""Tests for SlotWorker: clock-driven slot harvesting + decoder lifecycle.
...
def _make_worker(tmpdir, *, mode="ft8", cadence=15.0, ring=None,
                 keep_wav=False, decoder_path="/usr/local/bin/decode_ft8"):
    ...
    def test_tick_writes_wav_when_slot_complete(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            worker, clock, ring, box = _make_worker(
                tmpdir, keep_wav=True, decoder_path="/nonexistent/decode_ft8")
