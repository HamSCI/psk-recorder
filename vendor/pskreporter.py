...
class PskReporter(object):
    ...
    # Read interval from PSKREPORTER_INTERVAL env so deployments can
    # tune the upload cadence without forking.  The 180 s upstream
    # default is too slow for high-rate FT8/FT4 receivers — one batch
    # can contain thousands of spots, exceeding UDP MTU; WSJT-X has
    # also dropped its default to ~30 s.  Operators who want the old
    # cadence leave the env var unset.
    interval = int(os.environ.get("PSKREPORTER_INTERVAL", "180"))
    ...
    def spot(
        ...
        with self.spotLock:
            # PSKREPORTER_NO_DEDUP env var disables the 20-min
            # (callsign, mode, freq±10kHz) dedup check so every raw
            # decoded spot reaches the upload buffer.  Default (env
            # unset) preserves the historical protocol-conformant
            # dedup behavior — receivers running real-time decoders
            # can opt out for forensic analysis or to feed downstream
            # consumers that do their own dedup.
            _skip_dedup = bool(os.environ.get("PSKREPORTER_NO_DEDUP"))
            if (not _skip_dedup
                    and not spot["upload_all"]
                    and any(x for x in self.getOldSpots()
                            if self.spotEquals(spot, x))):
                # dupe
                pass
            else:
                self.addSpot(spot)
